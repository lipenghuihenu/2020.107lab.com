...
jQuery(function ($) { "use strict";
	...
	$('#contact-submit').click(function (e) {
		...
		if (phone.length == 0 || pattern.test(phone) == false) {
			var error = true;
			$('#phone').css("border-color", "#D8000C");
		} else {
		...
		if (error == false) {
			...
			$.post(url, $("#contact-form").serialize(), function (result) {
				...
				if (result == "1") {
					...
					//$('#mail-fail').attr('value', 'Sorry, Please do not repeat the submission.');
					$('#mail-fail0').fadeIn(500);
